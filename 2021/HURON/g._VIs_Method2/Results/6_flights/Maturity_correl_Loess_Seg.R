...
df_LOESS <- read.csv("I:/My Drive/UAS_Beans/Beans_Maturity/2021/HURON_Mat/g._VIs_Method2/Analysis/6_flights/Mat_LOESS_GLI_mean_0.06.csv")
df_SEG <- read.csv("I:/My Drive/UAS_Beans/Beans_Maturity/2021/HURON_Mat/g._VIs_Method2/Analysis/6_flights/Mat_SEG_GLI_mean_0.06.csv")
...
metrics.sum1 <- df_GT_LOESS_SEG %>%
  ...
  mutate(Dataset = c("LOESS_2021_HURON_6_fly")) %>% 
  print()
...
metrics.sum2 <- df_GT_LOESS_SEG %>%
  ...
  metrics_summary( obs = GT, pred = Mat_SEG_GLI_mean_0.06,
                   type = "regression")  %>%
  filter(Metric %in% c("r","MAE","MSE", "R2")) %>%
  mutate(Dataset = c("SEG_2021_HURON_6_fly")) %>% 
  print()
...
write.csv(data_metric_final, "Metric_2021_HURON.csv", row.names = F)
